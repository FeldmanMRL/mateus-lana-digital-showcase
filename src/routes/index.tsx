  ...
  return (
    <main className="mesh-bg min-h-screen text-foreground">
      ...
      <section id="expertise" className="relative z-10 mx-auto max-w-7xl px-5 py-28 lg:px-8">
        ...
        <div className="mt-14 grid gap-4 md:grid-cols-2">{skillGroups.map((skill, i)=><TiltCard key={skill.name} className="reveal glass-panel skill-card rounded-lg p-6" ><div className="flex items-start justify-between gap-4"><div><p className="text-lg font-bold">{skill.name}</p><p className="mt-2 text-xs leading-5 text-muted-foreground">{skill.tools}</p></div><span className="text-3xl font-extrabold text-primary"><AnimatedNumber value={skill.value} suffix="%"/></span></div><div className="mt-7 h-1 overflow-hidden rounded-full bg-muted"><div className="skill-progress h-full rounded-full bg-primary" style={{ "--skill-width":`${skill.value}%`, "--skill-delay":`${i*120}ms` } as React.CSSProperties}/></div></TiltCard>)}</div>
        <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{competencies.map((item)=><TiltCard key={item.title} className="reveal glass-panel rounded-lg p-6"><item.icon className="size-6 text-primary"/><h3 className="mt-8 font-bold">{item.title}</h3><p className="prose-justify mt-3 text-xs leading-6 text-muted-foreground">{item.text}</p></TiltCard>)}</div>
      </section>
