    ...
    <main className="mesh-bg min-h-screen text-foreground">
      ...
      <section ref={heroRef} onMouseMove={moveHero} onMouseLeave={(event) => { event.currentTarget.style.setProperty("--hero-x", "0"); event.currentTarget.style.setProperty("--hero-y", "0"); }} id="inicio" className="hero-depth relative z-10 mx-auto grid min-h-[min(94svh,960px)] max-w-7xl items-center gap-12 px-5 pb-20 pt-32 lg:grid-cols-[1.22fr_.78fr] lg:px-8">
        <div className="hero-copy reveal reveal-left max-w-4xl">
          ...
          <p className="mt-9 max-w-3xl border-l border-primary/60 pl-5 text-[clamp(1.15rem,2.1vw,1.8rem)] font-medium leading-tight text-foreground">Especialista em Marketing, Comunicação <span className="text-muted-foreground">& Audiovisual.</span></p>
        ...
        <div className="hero-portrait reveal reveal-scale relative mx-auto w-full max-w-md lg:mr-0" style={{ transitionDelay:"120ms" }}>
          <div className="portrait-frame glass-panel neon-shadow relative overflow-hidden rounded-lg p-2"><div className="spotlight-portrait relative aspect-[4/5] overflow-hidden rounded-md" onMouseMove={moveSpotlight} onMouseLeave={(e)=>e.currentTarget.style.setProperty("--spot-size","0px")}><img src={portraitAsset.url} alt="Retrato profissional de Mateus Lana" className="size-full object-cover object-top"/><img src={portraitAsset.url} alt="" aria-hidden className="spotlight-grayscale size-full object-cover object-top"/></div><div className="absolute inset-x-5 bottom-5 flex items-end justify-between rounded-md border border-border bg-background/80 p-4 backdrop-blur-xl"><div><p className="text-[10px] font-bold text-primary">DIREÇÃO CRIATIVA</p><p className="mt-1 text-sm font-semibold">27 anos · Minas Gerais</p></div><BadgeCheck className="size-5 text-primary"/></div></div>
        </div>
      </section>
      ...
      <section aria-label="Destaques" className="relative z-10 border-y border-border bg-surface/45"><div className="stats-grid mx-auto grid max-w-7xl grid-cols-2 divide-x divide-y divide-border px-0 sm:grid-cols-4 sm:divide-y-0 lg:px-0">{[["03+","anos em comunicação"],["04","núcleos de expertise"],["C1","espanhol"],["B1","inglês"]].map(([n,l])=><div key={l} className="stats-item reveal px-5 py-8 text-center sm:text-left"><strong className="text-3xl text-primary">{n}</strong><p className="mt-1 text-[10px] font-bold uppercase text-muted-foreground">{l}</p></div>)}</div></section>

      <section id="expertise" className="section-shell relative z-10 mx-auto max-w-7xl px-5 lg:px-8">
        <SectionTitle index="01" label="Domínio técnico" title="Ferramentas que transformam intenção em impacto." />
        <div className="skill-grid mt-14 grid gap-4 md:grid-cols-2">{skillGroups.map((skill, i)=><TiltCard key={skill.name} className="reveal reveal-scale glass-panel skill-card rounded-lg p-7" ><div className="flex items-start justify-between gap-4"><div><p className="text-lg font-bold">{skill.name}</p><p className="mt-2 text-xs leading-5 text-muted-foreground">{skill.tools}</p></div><span className="skill-number text-3xl font-extrabold text-primary"><AnimatedNumber value={skill.value} suffix="%"/></span></div><div className="mt-7 h-1 overflow-hidden rounded-full bg-muted"><div className="skill-progress h-full rounded-full bg-primary" style={{ "--skill-width":`${skill.value}%`, "--skill-delay":`${i*120}ms` } as React.CSSProperties}/></div></TiltCard>)}</div>
      ...
      <section id="experiencia" className="section-band relative z-10 border-y border-border bg-surface/40"><div className="mx-auto max-w-7xl px-5 lg:px-8"><SectionTitle index="02" label="Trajetória" title="Experiência construída no território."/><div ref={timelineRef} className="timeline-track relative mt-16 max-w-4xl">{[
      ...
      <section className="section-shell relative z-10 mx-auto max-w-7xl px-5 lg:px-8"><SectionTitle index="03" label="Formação" title="Conhecimento em movimento."/><div className="education-panel mt-14 overflow-hidden rounded-lg border border-border bg-border">{[
      ...
      <section id="projetos" className="section-band relative z-10 border-y border-border bg-surface/40"><div className="mx-auto max-w-7xl px-5 lg:px-8"><SectionTitle index="04" label="Portfólio selecionado" title="Comunicação que ocupa espaço."/><div className="project-grid mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-12">{projects.map((project, index)=><TiltCard key={project.title} className={`reveal reveal-scale project-card group relative cursor-pointer overflow-hidden rounded-lg border border-border bg-card ${index === 0 || index === 5 ? "lg:col-span-6" : "lg:col-span-3"}`} ><button onClick={()=>setSelected(project)} className="block w-full text-left" aria-label={`Abrir projeto ${project.title}`}><div className={`relative overflow-hidden ${index === 0 || index === 5 ? "aspect-[16/10]" : "aspect-[4/5]"}`}><img src={project.image} alt={`Peça do projeto ${project.title}`} loading="lazy" className="size-full object-cover transition-transform duration-700 group-hover:scale-105"/><span className="project-play absolute right-4 top-4 grid size-10 place-items-center rounded-full border border-border bg-background/80 text-primary backdrop-blur-md"><Play className="size-4 fill-current"/></span><div className="project-caption absolute inset-x-0 bottom-0 bg-gradient-to-t from-background via-background/85 to-transparent p-5 pt-24"><p className="text-[10px] font-bold uppercase text-primary">{project.category}</p><h3 className="mt-2 text-lg font-bold">{project.title}</h3><p className="mt-1 text-xs text-muted-foreground">{project.label}</p></div></div></button></TiltCard>)}</div></div></section>

      <section id="contato" className="section-shell relative z-10 mx-auto grid max-w-7xl gap-14 px-5 lg:grid-cols-[.8fr_1.2fr] lg:px-8"><div className="reveal"><p className="text-xs font-bold text-primary">05 / CONTATO</p><h2 className="mt-5 text-4xl font-extrabold leading-tight md:text-6xl">Uma boa ideia merece ganhar o mundo.</h2><p className="prose-justify mt-5 max-w-md text-sm leading-7 text-muted-foreground">Projetos de marca, campanhas, audiovisual e comunicação territorial começam com uma conversa.</p><div className="social-links mt-10 flex gap-3">{socialLinks.map(({ Icon, label })=><a key={label} href="#contato" aria-label={label} title={label} className="glass-panel grid size-11 place-items-center rounded-md text-muted-foreground transition-all hover:-translate-y-1 hover:border-primary/40 hover:text-primary"><Icon className="size-4"/></a>)}</div></div><form onSubmit={submit} className="contact-panel reveal glass-panel rounded-lg p-6 md:p-8"><div className="grid gap-5 md:grid-cols-2"><Field label="Nome" name="name" placeholder="Seu nome"/><Field label="E-mail" name="email" type="email" placeholder="voce@empresa.com"/><div className="md:col-span-2"><label className="text-xs font-bold text-muted-foreground" htmlFor="message">Mensagem</label><textarea id="message" name="message" required minLength={10} rows={5} placeholder="Conte um pouco sobre o projeto" className="mt-2 w-full resize-none rounded-md border border-input bg-background/55 px-4 py-3 text-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/15"/></div></div><button type="submit" className="shimmer-button mt-6 inline-flex w-full items-center justify-center gap-2 rounded-md bg-primary px-6 py-3.5 text-sm font-extrabold text-primary-foreground">{sent?"Mensagem preparada":"Enviar mensagem"}<ArrowUpRight className="size-4"/></button>{sent&&<p role="status" className="mt-3 text-center text-xs text-primary">Tudo certo — seus dados foram validados.</p>}</form></section>
...
function SectionTitle({ index, label, title }: { index: string; label: string; title: string }) {
  return <div className="section-title reveal grid gap-5 md:grid-cols-[.32fr_1fr]"><p className="flex items-center gap-3 text-xs font-bold text-primary"><span>{index}</span><span className="h-px w-8 bg-primary/45"/>{label.toUpperCase()}</p><h2 className="max-w-3xl text-3xl font-extrabold leading-tight md:text-5xl">{title}</h2></div>;
}
